...
const SubmitDPR = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let { data: projectData, error: projectErr } = await supabase
        .from("projects")
        .select("id")
      ...
      if (!projectId) {
        const { data: newProj, error: newProjErr } = await supabase
          .from("projects")
          .insert({ name: formData.projectName, start_date: formData.date, user_id: userId })
          .select()
          .single();
      ...
      // Use upsert to avoid duplicate entries for the same project, date, and user
      const { error: upsertErr } = await supabase.from("daily_reports").upsert(payload, {
